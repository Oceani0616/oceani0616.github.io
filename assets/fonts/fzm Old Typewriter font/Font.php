...
/*
  ...
  License: Copyright (C) 2012-2022 FontsForWeb
 */
ini_set('display_errors',0);
class FontPlugin {
    public $pluginName = 'Font';
    public $pageURL = '';
    public $baseUrl = 'fontsforweb.com';
    ...
    function __construct() {
        $this->pageURL = get_bloginfo('wpurl');
        ...
        // Font Browser
        wp_register_script('font-browser', plugins_url('/js/font_browser.js?pver=' . $this->version, __FILE__));
        //jquery font plugin
	...
	*/
	function font_activate ($update) {
		...
		if(!$this->get_plugin_version()) {
			$this->file_get_contents2($this->baseUrl . '/api/init?blogurl=' . urlencode($this->pageURL) . '&version=' . $this->version . '&action=INSTALLING');
		} else if($update) { //if updating
			$this->file_get_contents2($this->baseUrl . '/api/init?blogurl=' . urlencode($this->pageURL) . '&version=' . $this->version . '&action=UPDATING');
		} else {
			$this->file_get_contents2($this->baseUrl . '/api/init?blogurl=' . urlencode($this->pageURL) . '&version=' . $this->version . '&action=REACTIVATING');
		}
	...
	*/
    ...
    function getAllRules($presetsArr) {
		...
		$gotFontFace = $this->file_get_contents2($this->baseUrl . '/api/getallfontfaces?blogurl=' . urlencode($this->pageURL), $postData);
        return $gotFontFace;
    }
    //add task to a list
    ...
    function executeCSSGenerationTasks() {
        ...
        if (!isset($this->options['generatedPostsCSS'][$dbKey])) {
            $gotFontFace = $this->file_get_contents2($this->baseUrl . '/api/getpostfontfaces?blogurl=' . urlencode($this->pageURL) . '&' . $queryStr);
        ...
        switch ($this->downloadMethod) {
            case 'fopen':
				...
				if($fileContents) {
					...
					// Note: there was a null param in the middle
    ...
    function admin_menu() {
        ...
        wp_enqueue_script('font-browser');
	...
	function addAdminScripts () {
		?>
            <script type="text/javascript">
				...
				var fontBlogUrl = '<?php echo $this->pageURL; ?>';
    ...
    function admin_options_page() {
        echo '
			<div class="wrap">
				...
				<a href="' . $this->pageURL . '" target="_blank">Open your home screen in new tab.</a>
